...
// ─────────────────────────────────────────────────────────────────────────────
// EMOTIONAL DAMAGE SCAN — scrolls into view as a fake AI scan before FriendshipExpose
// ─────────────────────────────────────────────────────────────────────────────
function EmotionalDamageScan({ members }: { members: any[] }) {
...
// ─────────────────────────────────────────────────────────────────────────────
function ExportArchiveButton({ tripId }: { tripId: string }) {
  const exportData = trpc.trips.exportData.useMutation();

  const handleExport = async () => {
    try {
      const payload = await exportData.mutateAsync({ tripId });
      const blob = new Blob([JSON.stringify(payload, null, 2)], { type: 'application/json' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `yaarlore-trip-${tripId.slice(0, 8)}.json`;
  ...
  return (
    <button
      ...
      style={{
        background: 'rgba(245,240,232,0.04)',
        border: '1px solid rgba(245,240,232,0.08)',
        color: exportData.isPending ? 'rgba(245,240,232,0.25)' : 'rgba(245,240,232,0.35)',
      }}
      onMouseEnter={e => {
        if (!exportData.isPending)
          (e.currentTarget as HTMLButtonElement).style.color = 'rgba(245,240,232,0.55)';
      }}
      onMouseLeave={e => {
        (e.currentTarget as HTMLButtonElement).style.color = exportData.isPending
          ? 'rgba(245,240,232,0.25)'
          : 'rgba(245,240,232,0.35)';
      }}
    >
      {exportData.isPending ? 'Exporting...' : 'Export Archive'}
    </button>
  );
}
