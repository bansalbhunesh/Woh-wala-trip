...
interface Props { tripId: string; inviteCode: string; lore: LoreJson; members: any[]; }

export default function PublicStoryClient({ tripId, inviteCode, lore, members }: Props) {
  ...
  const advance = () => {
    ...
    setDir('forward'); setAnimKey(k => k + 1); setIdx(i => i + 1); setSlamActive(false);
  };
  const retreat = () => {
    if (idx <= 0) return;
    setDir('backward'); setAnimKey(k => k + 1); setIdx(i => i - 1); setSlamActive(false);
  };

  useEffect(() => {
    if (current.type === 'cooked') { const t = setTimeout(() => setSlamActive(true), 200); return () => clearTimeout(t); }
    setSlamActive(false);
  ...
  return (
    <div
      ...
      onClick={e => { if (e.clientX < window.innerWidth * 0.33) retreat(); else advance(); }}
    ...
    >
      ...
      {/* Back to overview */}
      <button onClick={e => { e.stopPropagation(); router.push(`/t/${inviteCode}`); }}
      ...
      <div className="absolute top-6 left-4 z-50 flex items-center gap-1.5 px-3 py-1 rounded-full text-[7.5px] font-mono uppercase tracking-wider"
           style={{ background: 'rgba(45,158,139,0.12)', border: '1px solid rgba(45,158,139,0.25)', color: 'rgba(45,158,139,0.8)' }}>
        ✓ PUBLIC STORY
      </div>
      ...
      <style jsx>{`
        @keyframes public-slide-right { from { opacity:0.4; transform:translateX(60px) scale(0.96); } to { opacity:1; transform:translateX(0) scale(1); } }
        @keyframes public-slide-left  { from { opacity:0.4; transform:translateX(-60px) scale(0.96); } to { opacity:1; transform:translateX(0) scale(1); } }
        @keyframes slam { 0%{transform:scale(0.4);opacity:0} 60%{transform:scale(1.1);opacity:1} 80%{transform:scale(0.97)} 100%{transform:scale(1)} }
...
function SlideContent({ slide, slamActive, countedScore, inviteCode, tripId }: { slide: Slide; slamActive: boolean; countedScore: number; inviteCode: string; tripId: string }) {
  switch (slide.type) {
      ...
      return (
        <div className="text-center space-y-6 max-w-sm">
          ...
          <p className="text-2xl font-vibe font-bold uppercase tracking-tight text-white/90"
             style={{ animation: slamActive ? 'rise 0.5s ease 0.8s both' : 'none', opacity: slamActive ? undefined : 0 }}>
    ...
    case 'character': {
      ...
      return (
        <div className="text-center space-y-6 max-w-sm" style={{ animation: 'flip 0.55s cubic-bezier(0.16,1,0.3,1) 0.05s both' }}>
    ...
    case 'superlative': {
      ...
      return (
        <div className="space-y-6 max-w-sm" style={{ animation: 'rise 0.6s cubic-bezier(0.16,1,0.3,1) both' }}>
          ...
          <div className="w-12 h-0.5 bg-chill-accent rounded-full" style={{ boxShadow: '0 0 8px rgba(45,158,139,0.4)' }} />
          <p className="text-5xl font-vibe font-bold text-cooked-accent" style={{ textShadow: '0 0 20px rgba(255,77,77,0.25)', animation: 'slam 0.5s cubic-bezier(0.16,1,0.3,1) 0.2s both' }}>
            {s.winner_name}
          </p>
        </div>
      );
    ...
    case 'verdict':
      return (
        <div className="text-center space-y-10 max-w-sm">
          <div className="w-12 h-0.5 bg-chill-accent mx-auto rounded-full" style={{ animation: 'fade-in 0.4s ease both' }} />
          <p className="text-2xl font-cinematic italic text-white/80 leading-relaxed" style={{ animation: 'rise 0.7s cubic-bezier(0.16,1,0.3,1) 0.15s both', opacity: 0 }}>
