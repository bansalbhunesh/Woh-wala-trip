...
function getMoodConfig(score: number) {
  if (score >= 76)
    return { freq: 55, type: 'sawtooth' as OscillatorType, label: 'CHAOS', detune: -20 };
  if (score >= 51)
    return { freq: 80, type: 'triangle' as OscillatorType, label: 'TENSION', detune: -10 };
  if (score >= 26) return { freq: 110, type: 'sine' as OscillatorType, label: 'WARM', detune: 5 };
  return { freq: 136.1, type: 'sine' as OscillatorType, label: 'MYSTIC', detune: 20 };
}
