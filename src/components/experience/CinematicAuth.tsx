...
function PortalCanvas({ phase }: { phase: number }) {
  ...
  useEffect(() => {
    ...
    const RINGS = [
      ...
      { r: 224, segs: 18, gap: 0.32, speed: -0.3, width: 3.0, bright: 0.95 },
      ...
      { r: 44, segs: 8, gap: 0.48, speed: -2.1, width: 2.0, bright: 0.65 },
    ];
    ...
    function newSnitchTarget() {
      ...
      const band = 0.22;
      if (edge === 0)
        return { tx: margin + Math.random() * (1 - margin * 2), ty: margin + Math.random() * band };
      if (edge === 1)
        return {
          tx: margin + Math.random() * (1 - margin * 2),
          ty: 1 - margin - Math.random() * band,
        };
      if (edge === 2)
        return { tx: margin + Math.random() * band, ty: margin + Math.random() * (1 - margin * 2) };
      return {
        tx: 1 - margin - Math.random() * band,
        ty: margin + Math.random() * (1 - margin * 2),
      };
    }
    ...
    const drawSparks = () => {
      for (let i = sparks.length - 1; i >= 0; i--) {
        ...
        g.addColorStop(0, WHITE(a * 0.9));
        g.addColorStop(0.5, GOLD(a * 0.4));
    ...
    const draw = () => {
      ...
      if (portalScale > 0.02) {
        ...
        deep.addColorStop(0.4, `rgba(160,50,8,${0.1 * I})`);
        ...
        ctx.lineWidth = 22 * I;
        ctx.shadowBlur = 30;
        ctx.shadowColor = AMBER(0.4 * I);
        ctx.beginPath();
        ctx.arc(cx, cy, 260 * I, 0, Math.PI * 2);
        ctx.stroke();
        ctx.restore();
        ...
        ctx.fillStyle = cg;
        ctx.beginPath();
        ctx.arc(cx, cy, 38 * I, 0, Math.PI * 2);
        ctx.fill();
...
export default function CinematicAuth() {
  ...
  const phaseStyle = (target: number): React.CSSProperties => ({
    ...
    transform:
      phase === target ? 'translate3d(0, 0, 0) scale(1)' : 'translate3d(0, 16px, 0) scale(0.985)',
    filter: phase === target ? 'blur(0px)' : 'blur(4px)',
    transition:
      'opacity 0.65s cubic-bezier(0.16,1,0.3,1), transform 0.65s cubic-bezier(0.16,1,0.3,1), filter 0.65s cubic-bezier(0.16,1,0.3,1)',
    ...
    <div className="fixed inset-0 overflow-hidden" style={{ background: '#060604' }}>
        ...
        <div className="w-full max-w-md space-y-9">
          <div className="text-center space-y-3">
            ...
            <h1
              ...
              style={{
                ...
                filter: phase === 1 ? 'blur(0px)' : 'blur(6px)',
                transition:
                  'opacity 0.65s cubic-bezier(0.16,1,0.3,1) 0.12s, transform 0.65s cubic-bezier(0.16,1,0.3,1) 0.12s, filter 0.65s cubic-bezier(0.16,1,0.3,1) 0.12s',
                willChange: 'transform, opacity',
              }}
            >
              IDENTIFY
              <br />
              <em className="italic" style={{ color: '#FFA020' }}>
                YOURSELF
              </em>
            </h1>
            <p
          ...
          <div
            ...
            style={{
              ...
              filter: phase === 1 ? 'blur(0px)' : 'blur(6px)',
              transition:
                'opacity 0.55s cubic-bezier(0.16,1,0.3,1) 0.22s, transform 0.55s cubic-bezier(0.16,1,0.3,1) 0.22s, filter 0.55s cubic-bezier(0.16,1,0.3,1) 0.22s',
              willChange: 'transform, opacity',
            }}
          >
          ...
          <div
            ...
            style={{
              ...
              filter: phase === 1 ? 'blur(0px)' : 'blur(6px)',
              transition:
                'opacity 0.55s cubic-bezier(0.16,1,0.3,1) 0.32s, transform 0.55s cubic-bezier(0.16,1,0.3,1) 0.32s, filter 0.55s cubic-bezier(0.16,1,0.3,1) 0.32s',
              willChange: 'transform, opacity',
            }}
          >
            <button
              ...
              style={{
                ...
                transition:
                  'transform 0.4s cubic-bezier(0.16,1,0.3,1), background 0.3s ease, box-shadow 0.4s cubic-bezier(0.16,1,0.3,1)',
                willChange: 'transform, opacity',
              }}
              onMouseEnter={e => {
                const el = e.currentTarget as HTMLButtonElement;
                el.style.transform = 'translate3d(0,-2px,0)';
                el.style.boxShadow = '0 8px 40px rgba(255,77,77,0.25)';
                el.style.background = 'rgba(255,77,77,0.13)';
              }}
              onMouseLeave={e => {
                const el = e.currentTarget as HTMLButtonElement;
                ...
                <>
                  <div
                    style={{
                      ...
                      animation: 'wwt-spin 0.9s linear infinite',
                    }}
                  />{' '}
                  TRANSMITTING...
                </>
            ...
            {isDevMode && (
              <p
                className="text-center font-mono text-[7.5px] uppercase tracking-[0.3em]"
      ...
      <div style={phaseStyle(2)}>
        <div className="w-full max-w-md space-y-5">
              ...
              >
                {d && (
                  <div
                    ...
                    style={{
                      boxShadow: '0 0 14px rgba(255,77,77,0.3)',
                ...
                <input
                  ...
                  style={{
                    ...
                    transition:
                      'transform 0.35s cubic-bezier(0.16,1,0.3,1), border-color 0.3s cubic-bezier(0.16,1,0.3,1)',
          ...
          {verifyLoading && (
            <p
              className="text-center font-mono text-[8px] uppercase tracking-[0.5em]"
              style={{
                color: 'rgba(255,140,30,0.55)',
                animation: 'wwt-breathe-dot 1s ease-in-out infinite',
              }}
            >
              VERIFYING...
            </p>
      ...
      {error && (
        <div
          className="fixed bottom-16 left-0 right-0 flex justify-center z-20 pointer-events-none"
          style={{
