  ...
  return (
    <div className="relative group">
      ...
      <div className="relative z-10 flex flex-col items-center text-center">
        <CinematicText variant="eyebrow" className="mb-4">
          {label}
        </CinematicText>
...
export function TimelineArchive({
  ...
  events: { time: string; event: string; type: string }[];
}) {
  return (
    <div className="space-y-0 border-l border-white/5 ml-4">
      {events.map((e, i) => (
        <div key={i} className="pl-10 py-10 relative group">
          ...
          <CinematicText variant="eyebrow" className="mb-2 text-white/40">
            {e.time}
          </CinematicText>
          <CinematicText
            variant="heading"
            className="text-2xl mb-4 tracking-normal normal-case italic font-medium text-white/90 leading-tight"
          >
            "{e.event}"
          </CinematicText>
          <div className="text-[10px] uppercase tracking-widest text-white/20 font-bold">
            Documented Phase · {e.type}
          </div>
        </div>
      ))}
    </div>
